...
export class UsersService {
  ...
  async updateToken(
   ...
   user = await this.usersRepository.findOne({where:{id:id}})
    this.queueClients.notifyAllServicesOfUpdatedToken(user);
    return user
  }
  ...
  async updatePin(
    ...
    let user = await this.getById(id);
   const newUser = await this.usersRepository.update(id, {
      hashedPurchasePin: pin,
    });
   user = await this.usersRepository.findOne({where:{id:id}})
    this.queueClients.notifyAllServicesOfUpdatedPin(user);
    return user
  }
  ...
  async changePin(
    ...
    let user = await this.getById(id);
    if(oldPin !==user.hashedPurchasePin){
   ...
   user = await this.usersRepository.findOne({where:{id:id}})
    this.queueClients.notifyAllServicesOfChangePin(user);
    return user
  }
  /**
  ...
  async updatePurchasePinWithCode(id:string,code: string, pin: string, pinConfirmation: string) {
    const confirmationCode = await this.confirmationCodeService.getConfirmationCode(code)
    if (pin != pinConfirmation) throw new BadRequestException("The chosen pin and the confirmation pin must match. Enter it again")

    let user = await this.getByPhoneNumber(confirmationCode.phoneNumber)
    ...
    user = await this.usersRepository.findOne({where:{id:id}})
    this.queueClients.notifyAllServicesOfChangePinWithCode(user);
